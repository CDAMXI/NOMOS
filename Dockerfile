# PLUTO — ASP.NET Core (.NET 10) API + web front end, backed by Supabase PostgreSQL.
...
COPY Pluto.Domain/Pluto.Domain.csproj Pluto.Domain/
COPY Pluto.Application/Pluto.Application.csproj Pluto.Application/
COPY Pluto.Infrastructure/Pluto.Infrastructure.csproj Pluto.Infrastructure/
...
RUN dotnet publish Pluto.Api/Pluto.Api.csproj -c Release -o /app/publish --no-restore -p:UseSharedCompilation=false
