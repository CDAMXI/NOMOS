...
// anywhere with zero setup. Set ConnectionStrings__Pluto to a PostgreSQL/Supabase string for
// a persistent, production database.
// El nombre tecnico paso de Nomos a Pluto: se sigue leyendo la clave vieja para que el despliegue
// no dependa de renombrar la variable de entorno en el mismo instante. Si NINGUNA de las dos esta
// puesta, la app cae al SQLite local y arranca con la base VACIA, sin error visible: por eso el
// puente se queda hasta confirmar que ConnectionStrings__Pluto existe en el host.
var connectionString = builder.Configuration.GetConnectionString("Pluto")
    ?? builder.Configuration.GetConnectionString("Nomos")   // puente: retirar cuando Render tenga la nueva
    ?? "Data Source=pluto_local.db";
builder.Services.AddPluto(connectionString);


builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        options.Cookie.Name = "pluto_auth";
...
builder.Services.AddExceptionHandler<Pluto.Api.GlobalExceptionHandler>();
...
using (var scope = app.Services.CreateScope())
{
    try
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<PlutoDbContext>();
